...
from timm.data import create_transform, IMAGENET_DEFAULT_STD, IMAGENET_DEFAULT_MEAN
...
def build_loader(config):
    ...
    print(f"local rank {0} / global rank {dist.get_rank()} successfully build train dataset")

    dsets['source_val'], dsets['target_val'] = build_dataset(is_train=False, config=config)
    print(f"local rank {0} / global rank {dist.get_rank()} successfully build val dataset")
